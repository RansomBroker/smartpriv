...
export default function AbsensiAdmin() {
    const [state, setState] = useState({
        input: {},
        siswa: [
            { siswa_id: 1, tanggal: '', nama: 'Andre', h: false, i: false, s: false },
            { siswa_id: 2, tanggal: '', nama: 'Damara', h: false, i: false, s: false },
    ...
    return (
        <>
            <Row className="mb-4">
                ...
                <Col md={4}>
                    <Form.Select onChange={(e) => setSelectedGuru(e.target.value)} value={selectedGuru}>
                        <option value="">Pilih Guru untuk Rekap</option>
                        {Object.entries(guruLabel).map(([val, label]) => (
                            <option key={val} value={val}>{label}</option>
                        ))}
                    </Form.Select>
                </Col>
            ...
            <Row>
                ...
                <Col md={6}>
                    ...
                    {selectedGuru && (
                        <Card>
                            <Card.Body>
                                <h5>Rekap Absensi {guruLabel[selectedGuru]}</h5>
                                <p>Hadir: 22 hari</p>
                                <p>Izin: 3 hari</p>
                                <p>Sakit: 1 hari</p>
                            </Card.Body>
