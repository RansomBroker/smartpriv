      ...
      <Row className="g-4 mb-4">
        {stats.map((item, idx) => (
          <Col key={idx} md={3} style={{ flexBasis: "21.8%" }}>
            <Card
              style={{ backgroundColor: item.bgColor, padding: "20px" }}
                ...
                <PieChart>
                  ...
                  >
                    {penilaianData.map((entry, index) => (
                      <Cell
                        key={`cell-${index}`}
                        fill={COLORS_PENILAIAN[index % COLORS_PENILAIAN.length]}
                      />
                    ))}
                  </Pie>
                  <Tooltip />
                  ...
                  <PieChart>
                    ...
                    >
                      {lokasiData.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={COLORS_LOKASI[index % COLORS_LOKASI.length]}
                        />
                      ))}
                    </Pie>
                    <Tooltip />
